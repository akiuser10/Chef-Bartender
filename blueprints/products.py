...
def ingredients_master():
    try:
        ...
        # Filter secondary items by organization
        sec_org_filter = get_organization_filter(HomemadeIngredient)
        secondary_items = HomemadeIngredient.query.filter(sec_org_filter).all()
        ...
        for p in products:
            ...
            editor_name = 'Unknown'
            if p.creator:
                creator_name = p.creator.first_name + ' ' + p.creator.last_name if (p.creator.first_name and p.creator.last_name) else (p.creator.first_name or p.creator.username)
            if p.last_editor:
                editor_name = p.last_editor.first_name + ' ' + p.last_editor.last_name if (p.last_editor.first_name and p.last_editor.last_name) else (p.last_editor.first_name or p.last_editor.username)
        ...
        for sec in secondary_items:
            ...
            editor_name = 'Unknown'
            if sec.creator:
                creator_name = sec.creator.first_name + ' ' + sec.creator.last_name if (sec.creator.first_name and sec.creator.last_name) else (sec.creator.first_name or sec.creator.username)
            if sec.last_editor:
                editor_name = sec.last_editor.first_name + ' ' + sec.last_editor.last_name if (sec.last_editor.first_name and sec.last_editor.last_name) else (sec.last_editor.first_name or sec.last_editor.username)
            ...
            rows.append({
                ...
                'supplier': 'In-House',
                'category': sec.category or 'Secondary',
                'sub_category': sec.sub_category or 'Secondary Ingredient',
                ...
                'last_edited_at': sec.last_edited_at
            })
...
def edit_ingredient(id):
    ...
    from datetime import datetime
    org_filter = get_organization_filter(Product)
    product = Product.query.filter(org_filter).filter_by(id=id).first_or_404()
    ...
    if request.method == 'POST':
        ...
        product.last_edited_by = current_user.id
        product.last_edited_at = datetime.utcnow()
        ...
        if current_user.organisation and current_user.organisation.strip():
            product.organisation = current_user.organisation.strip()
...
def delete_all_ingredients():
    try:
        ensure_schema_updates()
        from utils.helpers import get_organization_filter
        # Delete all products from user's organization (not secondary ingredients)
        org_filter = get_organization_filter(Product)
        products = Product.query.filter(org_filter).all()
        count = len(products)
...
def delete_selected_ingredients():
    try:
        ...
        for item_id in selected_ids:
            try:
                product = Product.query.filter(org_filter).filter_by(id=int(item_id)).first()
                if product:
                    db.session.delete(product)
                    count += 1
...
def bulk_upload_products():
    ...
    for idx, row in df.iterrows():
        try:
            ...
            sub_category = clean_str(row.get(sub_cat_col)) if sub_cat_col else None
            
            # Normalize "Other" variations
            category_lower = (category or '').lower().strip()
            sub_category_lower = (sub_category or '').lower().strip()
            
            # Check if category or sub_category is missing, empty, or "Other"
            needs_categorization = (
                not category or 
                not sub_category or 
                category_lower in ['other', 'others', ''] or 
                sub_category_lower in ['other', 'others', '']
            )
            
            # If category or sub_category is missing/empty/Other, use AI categorization
            if needs_categorization:
                from utils.product_categorizer import categorize_product
                ai_category, ai_sub_category = categorize_product(description)
                
                # Use AI categorization if original is missing, empty, or "Other"
                if (not category or category_lower in ['other', 'others', '']) and ai_category:
                    category = ai_category
                elif not category:
                    category = 'Other'
                
                if (not sub_category or sub_category_lower in ['other', 'others', '']) and ai_sub_category:
