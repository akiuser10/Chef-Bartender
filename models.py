...
# -------------------------
# COLD STORAGE UNIT MODEL
# -------------------------
class ColdStorageUnit(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    unit_number = db.Column(db.String(50), nullable=False)  # User-assigned unit number/name
    unit_name = db.Column(db.String(200))  # Optional descriptive name
    unit_type = db.Column(db.String(50), nullable=False)  # 'Freezer', 'Refrigerator', 'Wine Chiller'
    min_temp = db.Column(db.Float)  # Minimum acceptable temperature (user-defined for Wine Chiller)
    max_temp = db.Column(db.Float)  # Maximum acceptable temperature (user-defined for Wine Chiller)
    organisation = db.Column(db.String(200))  # Organization name for sharing
    created_by = db.Column(db.Integer, db.ForeignKey('user.id'))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    is_active = db.Column(db.Boolean, default=True)  # Allow soft deletion
    creator = db.relationship('User', foreign_keys=[created_by], backref='created_units')
    temperature_logs = db.relationship('TemperatureLog', backref='unit', lazy=True, cascade='all, delete-orphan')
    
    def get_temp_range(self):
        """Get the acceptable temperature range based on unit type"""
        if self.unit_type == 'Freezer':
            return (-22.0, -12.0)  # -22°C to -12°C
        elif self.unit_type == 'Refrigerator':
            return (0.0, 4.0)  # 0°C to 4°C
        elif self.unit_type == 'Wine Chiller':
            # User-defined range
            return (self.min_temp, self.max_temp) if self.min_temp is not None and self.max_temp is not None else (None, None)
        return (None, None)
    
    def is_temp_valid(self, temperature):
        """Check if a temperature reading is within acceptable range"""
        min_temp, max_temp = self.get_temp_range()
        if min_temp is None or max_temp is None:
            return True  # Wine chiller without defined range - always valid
...
class TemperatureLog(db.Model):
    ...
    log_date = db.Column(db.Date, nullable=False)  # Specific day (Monday-Sunday)
    time_slot = db.Column(db.String(10), nullable=False)  # '10:00 AM', '02:00 PM', '06:00 PM', '10:00 PM'
    unit_id = db.Column(db.Integer, db.ForeignKey('cold_storage_unit.id'), nullable=False)
    temperature = db.Column(db.Float, nullable=False)  # Recorded temperature
    corrective_action_type = db.Column(db.String(100))  # Predefined corrective action type
    corrective_action = db.Column(db.Text)  # Detailed corrective action notes
    recheck_temperature = db.Column(db.Float)  # Temperature after corrective action
    corrective_action_time = db.Column(db.String(50))  # Time corrective action was completed
    status = db.Column(db.String(20), default='OK')  # 'OK' or 'OUT OF RANGE'
    checked_by = db.Column(db.String(200))  # Name of person who checked
    bar_name = db.Column(db.String(200))  # Bar/Outlet name
    location = db.Column(db.String(200))  # Location
    daily_verified = db.Column(db.Boolean, default=False)  # Daily verification checkbox
    daily_verified_by = db.Column(db.String(200))  # Person who verified daily
    daily_verified_date = db.Column(db.Date)  # Date of daily verification
    supervisor_name = db.Column(db.String(200))  # Supervisor/Manager name
    supervisor_signature = db.Column(db.String(200))  # Supervisor signature
    supervisor_date = db.Column(db.Date)  # Supervisor verification date
    is_locked = db.Column(db.Boolean, default=False)  # Lock after supervisor verification
    locked_at = db.Column(db.DateTime)  # When record was locked
    locked_by = db.Column(db.Integer, db.ForeignKey('user.id'))  # Who locked the record
    organisation = db.Column(db.String(200))  # Organization name for sharing
    created_by = db.Column(db.Integer, db.ForeignKey('user.id'))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    creator = db.relationship('User', foreign_keys=[created_by], backref='created_temperature_logs')
    locker = db.relationship('User', foreign_keys=[locked_by], backref='locked_temperature_logs')
    
    def is_temp_valid(self):
        """Check if the recorded temperature is within acceptable range"""
        if self.unit:
            return self.unit.is_temp_valid(self.temperature)
        return True
    
    def update_status(self):
        """Update status based on temperature validity"""
        if self.is_temp_valid():
            self.status = 'OK'
        else:
            self.status = 'OUT OF RANGE'
        return self.status
    
    def requires_corrective_action(self):
        """Check if corrective action is required"""
        return self.status == 'OUT OF RANGE' and (not self.corrective_action_type or not self.corrective_action)
    
    def __repr__(self):
        return f'<TemperatureLog {self.id}: {self.log_date} {self.time_slot} - Unit {self.unit_id} - {self.temperature}°C - {self.status}>'

# -------------------------
# WEEKLY TEMPERATURE LOG METADATA MODEL
# -------------------------
class WeeklyTemperatureLog(db.Model):
    """Metadata for weekly temperature logs - HACCP compliance"""
    id = db.Column(db.Integer, primary_key=True)
    week_start_date = db.Column(db.Date, nullable=False, unique=True)  # Monday of the week
    bar_name = db.Column(db.String(200), nullable=False)
    location = db.Column(db.String(200), nullable=False)
    checked_by = db.Column(db.String(200), nullable=False)
    supervisor_name = db.Column(db.String(200))
    supervisor_signature = db.Column(db.String(200))
    supervisor_date = db.Column(db.Date)
    is_locked = db.Column(db.Boolean, default=False)  # Lock after supervisor verification
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    creator = db.relationship('User', foreign_keys=[created_by], backref='created_weekly_logs')
    locker = db.relationship('User', foreign_keys=[locked_by], backref='locked_weekly_logs')
    
    def __repr__(self):
        return f'<WeeklyTemperatureLog {self.id}: Week of {self.week_start_date} - {"LOCKED" if self.is_locked else "UNLOCKED"}>'
