/**
 * Chopping Board Cleaning & Sanitisation Checklist - Frontend JavaScript
 * Daily checklist management for chopping board cleaning
 */
...
// Load Units
function loadUnits() {
    fetch('/checklist/bar/chopping-board/units')
...
function loadChecklistEntry() {
    ...
    fetch(`/checklist/bar/chopping-board/entries?unit_id=${unitId}&entry_date=${entryDate}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                currentEntry = data;
...
function loadChecklistPoints(unitId) {
    fetch(`/checklist/bar/chopping-board/points?unit_id=${unitId}`)
        .then(response => response.json())
        .then(data => {
...
function updateChecklistItem(pointId) {
    ...
    fetch('/checklist/bar/chopping-board/entries', {
        method: 'POST',
        headers: {
...
function handlePointFormSubmit(e) {
    ...
    fetch('/checklist/bar/chopping-board/points', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
function generatePDF() {
    ...
    fetch('/checklist/bar/chopping-board/pdf', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    .then(blob => {
        ...
        a.href = url;
        a.download = `BAR_ChoppingBoard_Checklist_${pdfMonth}_${pdfYear}.pdf`;
