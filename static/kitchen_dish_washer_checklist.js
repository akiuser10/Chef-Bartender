/**
 * Kitchen Dish Washer Checklist - Frontend JavaScript
...
function initializeEventListeners() {
    ...
    const finalRinseTemp = document.getElementById('final-rinse-temperature');
    
    if (washTemp) washTemp.addEventListener('input', validateHACCP);
    if (finalRinseTemp) finalRinseTemp.addEventListener('input', validateHACCP);
...
function validateHACCP() {
    ...
    const finalRinseTemp = parseFloat(document.getElementById('final-rinse-temperature').value);
    ...
    // HACCP Rules:
    // 1. Wash temperature ≥ 60°C
    // 2. Final rinse temperature ≥ 82°C
    ...
    if (washTemp < 60) {
        pass = false;
        reasons.push('Wash temperature must be ≥ 60°C');
    }
    
    if (finalRinseTemp < 82) {
        pass = false;
        reasons.push('Final rinse temperature must be ≥ 82°C');
    }
...
async function loadUnits() {
    try {
        const response = await fetch('/checklist/kitchen/dish-washer/units');
        if (response.ok) {
...
async function handleFormSubmit(e) {
    ...
    const finalRinseTemp = formData.get('final_rinse_temperature');
    const staffInitials = formData.get('staff_initials');
    
    if (!unitId || !entryDate || !entryTime || !washTemp || !finalRinseTemp || !staffInitials) {
    ...
    const finalRinseTempNum = parseFloat(finalRinseTemp);
    
    let pass = true;
    if (washTempNum < 60) pass = false;
    if (finalRinseTempNum < 82) pass = false;
    ...
    const data = {
        ...
        final_rinse_temperature: finalRinseTempNum,
        staff_initials: staffInitials,
        corrective_action: correctiveAction || null
    };
    
    try {
        const response = await fetch('/checklist/kitchen/dish-washer/entries', {
            method: 'POST',
            headers: {
        ...
        <tbody>
            ${recentEntries.map(entry => `
                <tr>
                    ...
                    <td>${entry.final_rinse_temperature}</td>
