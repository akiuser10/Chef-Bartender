/**
 * Ice Scoop Sanitation Monitor (Bar)
 */
...
function loadUnits() {
    fetch('/checklist/bar/ice-scoop/units')
        .then(res => res.json())
        .then(data => {
            units = data;
            renderUnits();
...
function loadChecklistEntry() {
    ...
    fetch(`/checklist/bar/ice-scoop/entries?unit_id=${unitId}&entry_date=${entryDate}`)
        .then(res => res.json())
        .then(data => {
            if (!data.success) {
...
function saveChecklistEntry() {
    ...
    fetch('/checklist/bar/ice-scoop/entries', {
        method: 'POST',
