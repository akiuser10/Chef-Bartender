/**
 * BAR Opening Checklist - Frontend JavaScript
 * Daily checklist management for bar opening shift
 */
...
// Load Units
function loadUnits() {
    fetch('/checklist/bar/opening/units')
...
function loadChecklistEntry() {
    ...
    fetch(`/checklist/bar/opening/entries?unit_id=${unitId}&entry_date=${entryDate}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                currentEntry = data;
...
function loadChecklistPoints(unitId) {
    fetch(`/checklist/bar/opening/points?unit_id=${unitId}`)
        .then(response => response.json())
        .then(data => {
...
function updateChecklistItem(pointId) {
    ...
    fetch('/checklist/bar/opening/entries', {
        method: 'POST',
        headers: {
...
function handlePointFormSubmit(e) {
    ...
    fetch('/checklist/bar/opening/points', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
function generatePDF() {
    ...
    fetch('/checklist/bar/opening/pdf', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    .then(blob => {
        ...
        a.href = url;
        a.download = `BAR_Opening_Checklist_${pdfMonth}_${pdfYear}.pdf`;
